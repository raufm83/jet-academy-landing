...
export default async function SinglePostPage({ params }: ISinglePostPageProps) {
  ...
  try {
    ...
    const offerTitle = data.title[locale] || "";
    const offerDescription = htmlToDescription(data.content[locale] || "");
    const offerImageUrl = getPostImageSrc(data.imageUrl, locale);
    const schema = offerSingleGraph({
      name: offerTitle,
      description: offerDescription,
      url: `${base}/offers/${params.slug}`,
      locale,
      breadcrumbItems: [
        { name: locale === "az" ? "Ana Səhifə" : "Home", url: base },
        { name: locale === "az" ? "Kampaniyalar" : "Special Offers", url: `${base}/offers` },
        { name: offerTitle, url: `${base}/offers/${params.slug}` },
      ],
...
export async function generateMetadata({
  ...
  try {
    ...
    const metaImageUrl = getPostImageSrc(data.imageUrl, locale);
