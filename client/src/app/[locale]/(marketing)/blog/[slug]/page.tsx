...
import { getPostDetails, getPostsByType, getAllPosts } from "@/utils/api/post";
...
export default async function SinglePostPage({ params }: ISinglePostPageProps) {
  ...
  try {
    ...
    try {
      ...
      relatedPosts = (items ?? []).filter((p) => p.id !== currentId).slice(0, 6);
    } catch {
      relatedPosts = [];
    }
    ...
    const postTitle = data.title[locale] || "";
    const postDescription = htmlToDescription(data.content[locale] || "");
    const postImageUrl = getPostImageSrc(data.imageUrl, locale);
    const schema = blogSingleGraph({
      headline: postTitle,
      ...
      breadcrumbItems: [
        ...
        { name: postTitle, url: `${base}/blog/${params.slug}` },
      ],
          ...
          <div className="relative container">
            <div className="flex flex-col lg:flex-row gap-8 py-20">
              <div className="w-full flex flex-col gap-8 lg:w-2/3">
                ...
                {/* Author card: only when post is blog AND author role is AUTHOR (not admin/staff etc.) */}
                {data.postType === PostType.BLOG && data.author?.role === "AUTHOR" && (
                  <PostAuthorCard
                    author={data.author}
                    authorLabel={locale === "az" ? "Bloq müəllifi" : "Blog author"}
                    locale={locale}
                  />
                )}
...
export async function generateMetadata({
  ...
  try {
    ...
    const azCanonical = `${baseUrl}/az/${postTypeUrl}/${params.slug}`;
